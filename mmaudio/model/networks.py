...
class MMAudio(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if v2:
            ...
            self.mask_input_proj = nn.Sequential(
        ...
        self.empty_mask_feat = nn.Parameter(
            torch.zeros(1, sync_dim), requires_grad=True
        )
    ...
    def initalize_mask_enc_weights(self):
        def _basic_init(m):
        ...
        self.mask_enc.apply(_basic_init)
    ...
    def preprocess_conditions(
        ...
        assert (
            mask_f.shape[1] == self._sync_seq_len
        ), f"{mask_f.shape=} {self._sync_seq_len=}"
        ...
        mask_f = mask_f.view(bs, num_sync_segments, 8, -1) + self.sync_pos_emb
        mask_f = mask_f.flatten(1, 2)  # (B, VN, D)
        ...
        mask_f = self.mask_input_proj(mask_f)  # (B, VN, D)
        ...
        mask_f = mask_f.transpose(1, 2)  # (B, D, VN)
        mask_f = F.interpolate(mask_f, size=self._latent_seq_len, mode="nearest-exact")
        mask_f = mask_f.transpose(1, 2)  # (B, N, D)
    ...
    def forward(
        ...
        t: (B,)
        """
        conditions = self.preprocess_conditions(clip_f, sync_f, text_f, mask_f)
    ...
    def get_empty_mask_sequence(self, bs: int) -> torch.Tensor:
        return self.empty_mask_feat.unsqueeze(0).expand(bs, self._sync_seq_len, -1)
...
def small_44k_avs(**kwargs) -> MMAudio:
    num_heads = 7
    return MMAudio(
        latent_dim=40,
        clip_dim=1024,
...
def medium_44k_avs(**kwargs) -> MMAudio:
    num_heads = 14
    return MMAudio(
        latent_dim=40,
        clip_dim=1024,
...
def large_44k_avs(**kwargs) -> MMAudio:
    num_heads = 14
    return MMAudio(
        latent_dim=40,
        clip_dim=1024,
...
def get_my_mmaudio(name: str, **kwargs) -> MMAudio:
    ...
    if name == "small_44k_avs":
        return small_44k_avs(**kwargs)
    if name == "medium_44k":
        return medium_44k(**kwargs)
    if name == "medium_44k_avs":
        return medium_44k_avs(**kwargs)
    if name == "large_44k":
        return large_44k(**kwargs)
    if name == "large_44k_v2":
        return large_44k_v2(**kwargs)
    if name == "large_44k_avs":
        return large_44k_avs(**kwargs)
