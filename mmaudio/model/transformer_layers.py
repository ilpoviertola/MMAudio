...
class JointBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.pre_only = pre_only
        self.mask_block = MMDitSingleBlock(
    ...
    def forward(
        ...
        m_qkv, m_mod = self.mask_block.pre_attention(mask_f, global_c, latent_rot)

        latent_len = latent.shape[1]
        clip_len = clip_f.shape[1]
        text_len = text_f.shape[1]

        # m_qkv = (m_qkv[0], m_qkv[1].detach(), m_qkv[2].detach())
        # x_qkv = (x_qkv[0].detach(), x_qkv[1], x_qkv[2])
        joint_q = torch.cat([m_qkv[0], c_qkv[0], t_qkv[0]], dim=2)
        joint_k = torch.cat([x_qkv[1], c_qkv[1], t_qkv[1]], dim=2)
        joint_v = torch.cat([x_qkv[2], c_qkv[2], t_qkv[2]], dim=2)
        # joint_qkv = [torch.cat([x_qkv[i], c_qkv[i], t_qkv[i]], dim=2) for i in range(3)]

        attn_out = attention(joint_q, joint_k, joint_v)
        ...
        if not self.pre_only:
            ...
            mask_f = self.mask_block.post_attention(mask_f, x_attn_out, m_mod)

        return latent, clip_f, text_f, mask_f
