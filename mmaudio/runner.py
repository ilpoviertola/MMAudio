...
from mmaudio.model.sequence_config import CONFIG_16K, CONFIG_44K, CONFIG_44K_AVS
...
class Runner:
    ...
    def __init__(
        ...
        if cfg.model.endswith("16k"):
            ...
            mode = "44k"
        elif cfg.model.endswith("44k_avs"):
            self.seq_cfg = CONFIG_44K_AVS
            mode = "44k_avs"
        else:
            raise ValueError(f"Unknown model: {cfg.model}")

        assert mode == "44k_avs", "Only 44k_avs is supported"
        ...
        if for_training:
            ...
            parameter_groups = get_parameter_groups(
                self.network, cfg, print_log=(local_rank == 0)
            )
    ...
    def train_fn(
        ...
        x0, x1, xt, (clip_f, sync_f, text_f, mask_f) = self.fm.get_x0_xt_c(
            x1, t, Cs=[clip_f, sync_f, text_f, mask_f], generator=self.rng
        )
    ...
    def train_pass(self, data, it: int = 0):
        ...
        with torch.amp.autocast("cuda", enabled=self.use_amp, dtype=torch.bfloat16):
            ...
            mask_f[~video_exist] = self.network.module.empty_mask_feat
            ...
            if it % self.log_extra_interval == 0:
                ...
                unmasked_mask_f = mask_f.clone()
            x1, loss, mean_loss, t = self.train_fn(
                clip_f, sync_f, text_f, mask_f, a_mean, a_std
            )
        ...
        with (
            ...
            try:
                if it % self.log_extra_interval == 0:
                    ...
                    mask_f = unmasked_mask_f[0:1]
                    conditions = self.network.module.preprocess_conditions(
                        clip_f, sync_f, text_f, mask_f
                    )
    ...
    def validation_pass(self, data, it: int = 0):
        ...
        with torch.amp.autocast("cuda", enabled=self.use_amp, dtype=torch.bfloat16):
            ...
            loss, mean_loss, t = self.val_fn(
                clip_f.clone(), sync_f.clone(), text_f.clone(), mask_f.clone(), x1
            )
