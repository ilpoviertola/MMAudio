...
from mmaudio.model.sequence_config import (
    CONFIG_16K,
    CONFIG_44K,
    CONFIG_44K_AVS,
    SequenceConfig,
...
class ModelConfig:
    ...
    def seq_cfg(self) -> SequenceConfig:
        if self.mode == "16k":
            ...
            return CONFIG_44K
        elif self.mode == "44k_avs":
            return CONFIG_44K_AVS
...
small_44k_avs = ModelConfig(
    model_name="small_44k_avs",
    model_path=Path("./output/small_avs/small_avs_last.pth"),
    vae_path=Path("./ext_weights/v1-44.pth"),
    bigvgan_16k_path=None,
    mode="44k_avs",
)
    ...
    *,
    ...
    if sync_video is not None and not image_input:
        ...
        sync_features = feature_utils.encode_video_with_sync(
            sync_video, batch_size=bs * sync_batch_size_multiplier
        )
    else:
    ...
    if mask_video is not None and not image_input:
        mask_video = mask_video.to(device, dtype, non_blocking=True)
        mask_features = net.mask_enc(mask_video)
    else:
        mask_features = net.get_empty_mask_sequence(bs)
    ...
    preprocessed_conditions = net.preprocess_conditions(
        clip_features, sync_features, text_features, mask_features
    )
...
def load_video(
    video_path: Path, duration_sec: float, load_all_frames: bool = True
) -> VideoInfo:
