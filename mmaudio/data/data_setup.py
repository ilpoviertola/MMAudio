...
def load_avs_data(cfg: DictConfig, data_cfg: DictConfig) -> Dataset:
    dataset = ExtractedAVS(
        tsv_path=data_cfg.tsv,
        data_dim=cfg.data_dim,
        premade_mmap_dir=data_cfg.memmap_dir,
    )
...
def setup_training_datasets(
    ...
    if cfg.example_train:
        ...
        dataset = load_avs_data(cfg, cfg.data.ExtractedAVS)  # type: ignore
    else:
...
def setup_test_datasets(cfg):
    if cfg.avs_train:
        dataset = load_avs_data(cfg, cfg.data.ExtractedAVS_test)
    else:
...
def setup_val_datasets(cfg: DictConfig) -> tuple[Dataset, DataLoader, DataLoader]:
    if cfg.example_train:
        ...
        dataset = load_avs_data(cfg, cfg.data.ExtractedAVS_val)
    else:
...
def setup_eval_dataset(
    ...
    elif dataset_name.startswith("avs"):
        dataset = AVSSemantic(
